              ...
              <div class="panel-body">
                ...
                <div class="col-md-12 col-12 final_step-btn">
                  ...
                  @if($genrals_settings->min_pur_amount != 0 && $genrals_settings->min_pur_amount != ' ')
                      ...
                      @if($t_compare_amount >= $genrals_settings->min_pur_amount)
                      <button id="final_step"
                          class="pull-right btn btn-primary">{{__('staticwords.ProccedtoPayment')}}</button>
                      @else
                      <button disabled id="final_step"
                          class="pull-right btn btn-primary">{{__('staticwords.ProccedtoPayment')}}</button>
                          <strong style="color: red;">The minimum purhcase amount is $ {{ $genrals_settings->min_pur_amount }} 
                        </strong>
                      @endif
                  @else
                  <button id="final_step"
                          ...
                          @if(pre_order_disable() == false)
                            ...
                            strstr($listcheckOutCurrency->payment_method,'paystack'))
                              ...
                              <form method="POST" action="{{ route('pay.via.paystack') }}" accept-charset="UTF-8"
                                ...
                                <input type="hidden" name="amount" class="total_secure_amount" value="{{ sprintf("%.2f",Crypt::decrypt($secure_amount)) }}">
                                {{-- required in kobo --}}
                    ...
                    <div class="cart-sub-total">
                      <div class="row">
                        ...
                        <div class="col-lg-8 col-8">
                          ...
                          @if($shippingChage)
                          $<span id="new_shpping"> {{$shippingChage}} </span>
                          @else
                            Free
                          @endif
                          </span>
...
<script>
  ...
  console.log("checkout",carttottotalal);
</script>
...
<script>
  ...
  function handlingcharge(id){
    ...
    $('.total_amount_pay').val(total_amount_pay);
